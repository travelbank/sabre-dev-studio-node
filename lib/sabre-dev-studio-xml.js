...
var SabreDevStudioXml = ( function() {
  function SabreDevStudioXml( options ){
    ...
    /**
     * Gets a quuote for a rental vehicle
     * @param object data The data we'll, pass through, must validate!
     * {
     *   airportCode: string, Airport IATA code (required)
     *   startDate: string, Date which they'll be starting their stay (required, moment format)
     *   endDate: string, Date which they'll be ending their stay (required, moment format)
     *   vehicleTypes: array, Array of vehicle types (optional, @todo)
     * }
     * @param function cb The node callback function
     */
    this.VehicleAvailability = function( data, cb ){
      // Make sure we've got a good data object
      if( !_.isPlainObject( data ) ){
        data = {};
      }
      // Go ahead and call the service now
      that._callService( 'OTA_VehAvailRateLLSRQ', data, function( err, body, full, response ){
        if( err ){
          return cb( err );
        }
        /** Check for errors in the XML body */
        err = _.deepGet( body, 'OTA_VehAvailRateRS[0].stl:ApplicationResults[0].stl:Error[0].stl:SystemSpecificResults[0].stl:Message[0]' );
        /** If we have an error, callback with it */
        if( err ){
          return cb( err.trim() );
        }
        /** Make sure we have a good status message */
        var status = _.deepGet( body, 'OTA_VehAvailRateRS[0].stl:ApplicationResults[0].$.status' );
        if( status != 'Complete' ){
          return cb( 'Unknown status returned from vehicle availability search: ' + status.trim() )
        }
        /** Get the rentals from the body content */
        var rentals = _.deepGet( body, 'OTA_VehAvailRateRS[0].VehAvailRSCore[0].VehVendorAvails[0].VehVendorAvail' );
        if( !_.isArray( rentals ) && rentals.length ){
          return cb( 'Did not get any rentals from rental availability search.' );
        }
        /** Ok, we have a valid array of rentals, let's just return the response from here */
        return cb( null, rentals, body );
      } );
    };
    /**
     * Gets actual rail fare quotes from the sabre api
     * @param object data The data we'll, pass through, must validate!
     * {
     *   date: string, date of travel (required, moment format)
     *   from: string, city code for departure location (required, @todo where does this come from?)
     *   to: string, city code for destination (required, @todo where does this come from?)
     * }
     * @param function cb The node callback function
     */
    this.RailAvailability = function( data, cb ){
      // Make sure we've got a good data object
      if( !_.isPlainObject( data ) ){
        data = {};
      }
      ...
      // Go ahead and call the service now
      that._callService( 'RailShopRQ', data, function( err, body, full, response ){
        if( err ){
          return cb( err );
        }
        /** Check for errors in the XML body */
        err = _.deepGet( body, 'rs:RailShopRS[0].rs:Errors[0].rcmn:Error[0]' );
        /** If we have an error, callback with it */
        if( err ){
          return cb( err._ );
        }
        /** Get the offers from the body content */
        var offers = _.deepGet( body, 'rs:RailShopRS[0].rs:OffersDetails[0].rs:Offers[0].rcmn:Offer' );
        if( !_.isArray( offers ) && offers.length ){
          return cb( 'Did not get any valid options from rail availability search.' );
        }
        /** Ok, we have a valid array of offers, let's just return the response from here */
        return cb( null, offers, body );
      } );
    };
    /**
     ...
     * @param object data The data we'll pass through, must validate!
     * {
     *   latitude: float, Latitude of location for search
     *   longitude: float, Longitude of location for search
     *   startDate: string, Date which they'll be starting their stay (required, moment format)
     *   endDate: string, Date which they'll be ending their stay (required, moment format)
     *   awardRating: int, Preferred hotel rating (optional)
     * }
     * @param function cb The node callback function
     */
    this.HotelAvailability = function( data, cb ){
      ...
      that._callService( 'OTA_HotelAvailLLSRQ', data, function( err, body, full, response ){
        ...
        if( status != 'Complete' ){
          return cb( 'Unknown status returned from hotel availability search: ' + status.trim() )
        }
        ...
        if( !_.isArray( hotels ) && hotels.length ){
          return cb( 'Did not get any hotels from hotel availability search.' );
        }
      ...
      this._getTemplate( service, function( err, template ){
        ...
        var requestCallback = function( err, response, body ){
          ...
          xml2js.parseString( body, function( err, object ){
            ...
            // @todo this a less-stupid way (its a special case for rail)
            if( _.isUndefined( body ) ){
              body = _.deepGet( object, 'SOAP-ENV:Envelope.SOAP-ENV:Body[0]' );
            }
